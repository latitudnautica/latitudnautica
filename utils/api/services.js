...
export const _delete = async (id, endpoint) => {
  return await axiosBase.delete(`${endpoint}/${id}`, {
...
export const _create = async (data, endpoint) => {
   ...
   */
  return await axiosBase.post(`/category/${endpoint}`, data, {
    headers: { Authorization: `Bearer ${Cookies.get("token")}` },
  });
};

export const _update = async (id, payload, endpoint) => {
  return await axiosBase.put(
    `${endpoint}/${id}`,
    { name: payload },
    { headers: { Authorization: `Bearer ${Cookies.get("token")}` } }
  );
